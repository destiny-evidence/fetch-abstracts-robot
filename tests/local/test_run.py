...
from far.local.run import AbstractRetrievalError, main
...
def test_main_no_abstracts_found(mocker, tmp_path, test_references):
    doi_list_file = tmp_path / "doi_list.txt"
    ...
    with pytest.raises(AbstractRetrievalError):
        main(doi_list=doi_list_file, output_directory=tmp_path / "output")
