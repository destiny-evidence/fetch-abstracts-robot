...
def get_device_flow(settings: Settings) -> str:
    """
    ...
    app = msal.PublicClientApplication(
        client_id=settings.destiny_robot_dev_auth_app_id, authority=authority
    )

    flow = app.initiate_device_flow(
        scopes=[f"api://{settings.destiny_repository_development_app_id}/.default"]
    )
...
def get_app_flow(settings: Settings) -> dict:
    ...
    app = msal.ConfidentialClientApplication(
        client_id=settings.destiny_robot_dev_auth_app_id,
        client_credential=settings.destiny_robot_dev_auth_secret.get_secret_value(),
        authority=authority,
    )

    return app.acquire_token_for_client(
        scopes=[f"api://{settings.destiny_repository_staging_app_id}/.default"]
    )
